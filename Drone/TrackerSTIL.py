...
class UdpMavSender:
    """Minimal MAVLink sender over UDP for external map GUI."""
    ...
    def send_heartbeat(self, armed=False):
        try:
            ...
            typ = mavutil.mavlink.MAV_TYPE_QUADROTOR
            base = 0
            if armed:
                base |= mavutil.mavlink.MAV_MODE_FLAG_SAFETY_ARMED
            msg = mavutil.mavlink.MAVLink_heartbeat_message(
                typ, ap, base, 0, mavutil.mavlink.MAV_STATE_ACTIVE
            )
            self._send(msg)
        except Exception:
            pass

    def send_global_position_int(self, lat, lon, alt_m, hdg_deg):
        try:
            msg = mavutil.mavlink.MAVLink_global_position_int_message(
                ...
                int(alt_m*1000),
                0, 0, 0,
                int(max(0.0, min(360.0, hdg_deg))*100)
            )
            self._send(msg)
        except Exception:
            pass

    def send_attitude(self, roll_deg, pitch_deg, yaw_deg):
        try:
    ...
    def send_vfr_hud(self, alt_m, gs_mps, hdg_deg):
        try:
            msg = mavutil.mavlink.MAVLink_vfr_hud_message(
                0.0, float(gs_mps), 0, int(hdg_deg), 0, float(alt_m), 0.0
            )
            self._send(msg)
        except Exception:
            pass

    def send_sys_status(self, volt_v, cur_a):
        try:
            vb = int(max(0.0, volt_v)*1000)
            cb = int(cur_a*100)
            msg = mavutil.mavlink.MAVLink_sys_status_message(
                0,0,0, 0,0,0, 0, vb, cb, -1, 0,0,0,0
            )
            self._send(msg)
        except Exception:
            pass
