...
- חיבור לשלט PS4 מתבצע בהתחלה בלבד
- כפתורים להחלפת מצב טיסה (STABILIZE, ACRO, GUIDED)
"""
...
class FakeSticks:
    def __init__(self, root):
        ...
        # Flight mode buttons
        lf_mode = ttk.LabelFrame(root, text="Flight Mode", padding=8)
        lf_mode.pack(fill="x", padx=8, pady=6)
        ttk.Button(lf_mode, text="STABILIZE", command=lambda: self.set_mode("STABILIZE")).pack(side="left", expand=True, fill="x", padx=4)
        ttk.Button(lf_mode, text="ACRO", command=lambda: self.set_mode("ACRO")).pack(side="left", expand=True, fill="x", padx=4)
        ttk.Button(lf_mode, text="GUIDED", command=lambda: self.set_mode("GUIDED")).pack(side="left", expand=True, fill="x", padx=4)
