...
# ==== תצורה בסיסית (אפשר לעדכן גם בפרמטרים/ENV) ====
MAV_PORT = "COM14"
MAV_BAUD = 115200

DEFAULT_SIM_IP   = os.getenv("SIM_IP", "127.0.0.1")  # יכול להיות IP או hostname
DEFAULT_SIM_PORT = int(os.getenv("SIM_PORT", "6000"))
DEFAULT_BCAST    = os.getenv("SIM_BROADCAST", "0") in ("1", "true", "True", "yes", "YES")
DEFAULT_NET_CIDR = os.getenv("SIM_NET")              # לדוגמה: "192.168.1.0/24"
DEFAULT_BIND_IP  = os.getenv("SIM_BIND")             # אם רוצים לכפות יציאה דרך ממשק מסוים
...
def _get_local_ip_guess() -> str:
    """ניסיון לשאוב את ה-IP המקומי של הממשק שדרכו יוצאים החוצה."""
    s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    try:
        # לא באמת שולחים ל-8.8.8.8; זה רק כדי שהקרנל יבחר ממשק וכתובת מקור
...
def _resolve_target(target: str, port: int, use_broadcast: bool, net_cidr: str|None):
    """מאחד: IP/hostname + Broadcast. מחזיר (dest_ip, dest_port, is_broadcast)."""
    if use_broadcast:
        # 1) אם הועברה רשת (/24 וכד') – נחשב כתובת broadcast ממנה
        if net_cidr:
            try:
                ...
                print("⚠️ SIM_NET לא תקין, ננסה ברירת מחדל:", e)

        # 2) אם ה- target כבר נראה כמו כתובת broadcast (מסתיים ב-.255) – נשתמש בו
        try:
            parts = target.split(".")
        ...
        # 3) ניסיון אוטומטי לפי /24 של ה-IP המקומי
        ...
        # יוניקאסט: hostname/IP רגיל
        try:
            dest_ip = socket.gethostbyname(target)
        except Exception:
            # אם resolve נכשל (למשל ללא DNS) – ננסה להשתמש ככתובת כמו שהיא
            dest_ip = target
...
def _build_socket(bind_ip: str|None, is_broadcast: bool):
    ...
    if bind_ip:
        # קושרים ל-IP מקומי מסוים (פורט 0 = אוטומטי)
        s.bind((bind_ip, 0))
    return s

parser = argparse.ArgumentParser(description="GUI → Isaac pose sender (UDP, hostname/broadcast ready)")
parser.add_argument("-t", "--target", default=DEFAULT_SIM_IP, help="יעד ה-UDP: IP או hostname (ברירת מחדל: %(default)s)")
parser.add_argument("-p", "--port",   default=DEFAULT_SIM_PORT, type=int, help="פורט UDP של הסימולציה (ברירת מחדל: %(default)s)")
parser.add_argument("-b", "--broadcast", action="store_true", default=DEFAULT_BCAST, help="שידור Broadcast (True/False)")
parser.add_argument("--net", default=DEFAULT_NET_CIDR, help="CIDR לרשת (למשל 192.168.1.0/24) כדי לחשב broadcast")
parser.add_argument("--bind", default=DEFAULT_BIND_IP, help="Bind ל-IP מקומי מסוים (למשל 192.168.1.197)")

# נחשב את היעד הסופי וה-socket כבר בהעלאה, כדי שהפונקציות ישתמשו בו
