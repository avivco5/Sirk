...
}T_UAVPositioningPositionDeviation;

typedef struct {
    E_UAVPositioningProperty property;
    T_UAVAttitude3d uavAttitude;  /*!< Specifies UAV attitude, unit: degree. */
    T_UAVPositioningPositionDeviation positionDeviation;  /*!< Specifies position deviation, unit: meter. */
}T_UAVPositioningPositionInfo;

T_UAVReturnCode UAV_Positioning_Init(void);
void UAV_Positioning_Deinit(void);

T_UAVReturnCode UAV_Positioning_GetPositioning_Sync(uint8_t eventCount, T_UAVPositioningEventInfo *eventInfo, T_UAVPositioningPositionInfo *positionInfo);
