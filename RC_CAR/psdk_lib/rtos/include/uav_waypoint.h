...
}T_UAVWAYPOINT_ACTION_STATE;

typedef int (*UAVWaypointMissionStateCallback_t)(T_UAVWAYPOINT_MISSION_STATE missionState);


typedef int (*UAVWaypointActionStateCallback_t)(T_UAVWAYPOINT_ACTION_STATE actionState);

/*
 * @brief Initialise waypoint module, and user should call this function.
 * before using waypoint features.
 */
T_UAVReturnCode UAV_Waypoint_Init(void);

/*
 * @brief Deinitialise waypoint module.
 */
T_UAVReturnCode UAV_Waypoint_Deinit(void);
...
T_UAVReturnCode UAV_Waypoint_Upload_kmz(const uint8_t *data, uint32_t size);

/*
 * @brief Execute the mission action.
 * @note This action should be called after uploading the kmz file.
 */
T_UAVReturnCode UAV_Waypoint_Action(const E_UAV_WAYPOINT_ACTION action, const int32_t msec);
 ...
 */
T_UAVReturnCode UAV_Waypoint_RegisterMissionStateCallback(UAVWaypointMissionStateCallback_t callback);
 ...
 * file or executing this mission action.
 */
T_UAVReturnCode UAV_Waypoint_RegisterActionStateCallback(UAVWaypointActionStateCallback_t callback);
